...
class Msummary extends CI_Model {
	...
	{
		$rs 	= $this->db->query("SELECT T02.member_name FROM summary_report_detail T01 LEFT JOIN member T02 ON T02.id = T01.driver_id WHERE T01.report_date = '".$tanggal."' AND T01.client_id = ".$id." GROUP BY T01.driver_id, T01.report_date");
			$result["total"] = $rs->num_rows();

		$rs	= $this->db->query("SELECT T02.member_name, CONCAT_WS(' ', T02.merk_mobil, T02.type_mobil) AS mobil, T02.city, FORMAT(ROUND(SUM(T01.total_km), 0), 0) AS total_km, FORMAT(ROUND(SUM(T01.total_saldo), 0), 0) AS total_saldo, FORMAT(ROUND(SUM(T01.dana_driver), 0), 0) AS dana_driver, FORMAT(ROUND(SUM(T01.dana_tempelad), 0), 0) AS dana_tempelad, FORMAT(ROUND(SUM(T01.total_viewer), 0), 0) AS total_viewer, T01.phone, T01.report_date FROM summary_report_detail T01 LEFT JOIN member T02 ON T02.id = T01.driver_id WHERE T01.report_date = '".$tanggal."' AND T01.client_id = ".$id." GROUP BY T01.driver_id, T01.report_date ORDER BY $sort $order LIMIT $offset,$limit");
	...
	{
		$query	= $this->db->query("SELECT T02.member_name, CONCAT_WS(' ', T02.merk_mobil, T02.type_mobil) AS mobil, T02.city, FORMAT(ROUND(T01.total_km, 0), 0) AS total_km, FORMAT(ROUND(T01.total_saldo, 0), 0) AS total_saldo, FORMAT(ROUND(T01.dana_driver, 0), 0) AS dana_driver, FORMAT(ROUND(T01.dana_tempelad, 0), 0) AS dana_tempelad, FORMAT(ROUND(T01.total_viewer, 0), 0) AS total_viewer FROM summary_report_detail T01 LEFT JOIN member T02 ON T02.id = T01.driver_id WHERE T01.report_date = '".$date."' AND T01.client_id = ".$client_id." ORDER BY T02.member_name ASC");
