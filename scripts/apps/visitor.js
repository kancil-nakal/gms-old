...
var $table = $('#listVisitor');
function listVisitor() {
	$table.bootstrapTable({
		method: 'GET',
		url: BASE_URL+'visitor/listvisitor',
		cache: false,
		search : true,
		pagination : true,
		striped: true,
		toolbar: '#toolbarVisitor',
		...
		columns: [
			...
			{ field: 'total', title: 'Total', align: 'left', sortable: false },
...
window.operateEvents = {
	'click .edit': function (e, value, row, index) {
		window.location = BASE_URL+'visitor/form/'+row.id;
	},
	'click .remove': function (e, value, row, index) {
		swal({
			...
			if (isConfirm) {
				$.ajax({
					type: 'GET',
					url: BASE_URL+'visitor/delete/'+row.id,
					dataType: 'json',
