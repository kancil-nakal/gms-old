...
function listMerk() {
	$table.bootstrapTable({
		...
		columns: [
			...
			{ field: 'merk_car', title: 'Merk Mobil', align: 'left', sortable: true },
			{ field: 'status', title: 'Enabled', align: 'center', width: '90px', formatter: statusFormat, events: operateStatus },
...
window.operateStatus = {
	'click .flagActive': function (e, value, row, index) {
		$.ajax({
			type: 'get',
			url: BASE_URL+'merk/status/'+row.id+'/'+row.status,
			dataType: 'json',
...
window.operateEvents = {
	'click .edit': function (e, value, row, index) {
		window.location = BASE_URL+'merk/form/'+row.id;
	},
	'click .remove': function (e, value, row, index) {
		swal({
			...
			if (isConfirm) {
				$.ajax({
					type: 'GET',
					url: BASE_URL+'merk/delete/'+row.id,
					dataType: 'json',
					success: function (data) {
						if (data.success == true) { swal({ title: "Success", text: "<b>" +row.brand_name+ ' ' +row.merk_car+ "</b> successfully deleted", timer: 2000, type: "success", showConfirmButton: false, html: true }); }
						else { swal({ title: "Failed", text: "<b>" +row.brand_name+ ' ' +row.merk_car+ "</b> failed to deleted", timer: 2000, type: "error", showConfirmButton: false, html: true }); }
					},
