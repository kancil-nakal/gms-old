...
var $table = $('#listActivity');
function listActivity() {
	$table.bootstrapTable({
		method: 'GET',
		url: BASE_URL+'activity/listactivity',
		cache: false,
		search : true,
		pagination : true,
		striped: true,
		...
		columns: [
			...
			{ field: 'description', title: 'Description', align: 'left', sortable: false },
...
window.operateEvents = {
	'click .edit': function (e, value, row, index) {
		window.location = BASE_URL+'activity/form/'+row.id;
	},
	'click .remove': function (e, value, row, index) {
		swal({
			...
			if (isConfirm) {
				$.ajax({
					type: 'GET',
					url: BASE_URL+'activity/delete/'+row.id,
					dataType: 'json',
