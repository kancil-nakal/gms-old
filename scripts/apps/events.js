...
var $table = $('#listEvents');
function listEvents() {
	$table.bootstrapTable({
		method: 'GET',
		url: BASE_URL+'events/listEvents',
		cache: false,
		search : true,
		pagination : true,
		striped: true,
		...
		sortOrder: 'desc',
		columns: [
			{ field: 'event_name', title: 'Events Name', align: 'left', sortable: true },
			{ field: 'date_start', title: 'Start', align: 'right' },
			{ field: 'date_end', title: 'End', align: 'right' },
...
window.operateStatus = {
	'click .flagActive': function (e, value, row, index) {
		$.ajax({
			type		: 'get',
			url			: BASE_URL+'events/status/'+row.id+'/'+row.status,
			dataType	: 'json',
...
window.operateEvents = {
	'click .edit': function (e, value, row, index) {
		window.location = BASE_URL+'events/form/'+row.id;
	},
	'click .remove': function (e, value, row, index) {
		swal({
			...
			if (isConfirm) {
				$.ajax({
					type		: 'GET',
					url			: BASE_URL+'events/delete/'+row.id,
					dataType	: 'json',
					success: function (data) {
