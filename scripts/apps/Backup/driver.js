(function ($) {
	...
	function listDriver() {
		...
		$table.bootstrapTable({
			...
			columns: [
				...
				{ field: 'user_status', title: 'Enabled', align: 'center', width: '80px', formatter: statusFormat, events: operateStatus },
				{ field: null, title: 'Action', align: 'center', width: '190px', formatter: operateFormatter, events: operateEvents }
			]
		});
	}
	...
	window.operateStatus = {
		'click .flagActive': function (e, value, row, index) {
			$.ajax({
				type		: 'get',
				url			: BASE_URL+'driver/status/'+row.id+'/'+row.user_status,
				dataType	: 'json',
