(function ($) {
	...
	window.operateStatus = {
		'click .flagActive': function (e, value, row, index) {
			$.ajax({
				type		: 'get',
				url			: BASE_URL+'apti/status/'+row.id+'/'+row.ta_status,
				dataType	: 'json',
				success		: function(data) {
					if (data.msg == 'success') {
						$("#listApti").bootstrapTable('refresh', true);
					} else {
						swal({
							title	: "Error",
							text	: "Failed",
	...
	function operateFormatter(value, row, index) {
		return [
			'<div class="btn-groupss">',
				'<button type="button" class="btn btn-success edit" title="Edit" style="margin-right:5px;width:80px;"><i class="fa fa-edit"></i> Edit</button>',
				'<button type="button" class="btn btn-danger remove" title="Delete" style="width:80px;"><i class="fa fa-trash-o"></i> Delete</button>',
			'</div>'
		].join('');
	}
	...
	window.operateEvents = {
		'click .edit': function (e, value, row, index) {
			window.location = BASE_URL+'apti/form/'+row.id;
		},
		'click .remove': function (e, value, row, index) {
			swal({
				...
				if (isConfirm) {
					$.ajax({
						type		: 'GET',
						url			: BASE_URL+'apti/delete/'+row.id,
						dataType	: 'json',
						success: function (data) {
						...
						$("#listApti").bootstrapTable('refresh');
					});
				}
			});
		}
