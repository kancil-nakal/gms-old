...
function listClient() {
	...
	$table.bootstrapTable({
		...
		columns: [
			...
			{ field: 'saldo', title: 'Saldo', align: 'right' },
...
window.operateStatus = {
	'click .flagActive': function (e, value, row, index) {
		$.ajax({
			type		: 'get',
			url			: BASE_URL+'client/status/'+row.id+'/'+row.ta_status,
			dataType	: 'json',
...
function getArea(id) {
	...
	$table.bootstrapTable({
		...
		pageSize: 5,
		columns: [
			{ field: 'city', title: 'AREA', align: 'left', formatter: selectFormatterArea, events: selectEventsArea },
...
function getDashboard(area, id) {
	...
	$table.bootstrapTable({
		...
		columns: [
			...
			{ field: null, title: 'Detail', align: 'center', formatter: operateFormatters }
		]
	});
}
