...
<?php foreach ($getData as $row) { ?>
	<div class="p-a white lt box-shadow">
		<div class="row">
			<div class="col-sm-6">
				<h4 class="m-b-0 _300">Profile</h4>
			</div>
		</div>
	</div>
			...
			<form action="<?php echo base_url('operator/edit'); ?>" method="post" enctype="multipart/form-data">
				<div class="box">
						...
						<div class="row">
							<div class="col-sm-5">
								<div class="form-group row">
									...
									if( $row->ta_image != '' ) {
										?>
										<label class="col-sm-4"><img id="thumbnil" style="width:100px;height:127px;" src="<?php echo base_url(); ?>assets/images/photo/operator/<?php echo $row->user_image ;?>" alt="image"/></label>
										<?php
									} else {
										?>
					...
					<div class="dker p-a text-left">
						...
						<input type="hidden" name="status" value="<?php echo $row->ta_status ;?>"></input>
						<button type="submit" class="btn btn-fw info">Submit</button>
						<a href="javascript:history.back();" class="btn btn-fw danger">Back</a>
					</div>
				</div>
			</form>
		</div>
